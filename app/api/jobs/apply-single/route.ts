...
export async function POST(req: Request) {
    const { id, cvId, template } = await req.json();
    ...
    if (job.length === 0 || cv.length === 0 || userRecord.length === 0) {
        return NextResponse.json(
            { message: "Job, CV and User not found" },
            { status: 404 }
        );
    }
    ...
    for (const recipient of job[0].emails) {
        await sendEmail(recipient, email.subject, email.body, pdfBuffer, userRecord[0].name as string, userRecord[0].email as string);
    }
    ...
    return NextResponse.json({ status: "ok", message: "Apply to job successfully" });
}
